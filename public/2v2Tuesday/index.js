...
socket.onmessage = event => {
	...
	console.log(data.tourney.manager.bools.scoreVisible)
	if(scoreVisibleTemp !== data.tourney.manager.bools.scoreVisible) {
		...
		if(scoreVisibleTemp) {
			...
			statsCont.style.transform = "translateX(0px)"
			...
			statsCont.style.transform = "translateX(-500px)"
	...
	// Update player cards
	if (player1NameTemp !== data.tourney.ipcClients[0].spectating.name || player2NameTemp !== data.tourney.ipcClients[1].spectating.name || player3NameTemp !== data.tourney.ipcClients[2].spectating.name || player4NameTemp !== data.tourney.ipcClients[3].spectating.name ) {
		...
		for (i = 0; i < players.length; i++) {
			...
			let pfpUrl =  "http://s.ppy.sh/a/" + data.tourney.ipcClients[i].spectating.userID;
  ...
  if(tempHP !== data.menu.bm.stats.HP) {
	...
	if(scoreVisibleTemp) {
		...
		if(scoreRedTemp > scoreBlueTemp) {
			...
			playScoreRed.style.filter = 'transformY(-10px)';

			playScoreBlue.style.backgroundColor = '#8D0065';
			playScoreRed.style.filter = 'transformY(0)';
